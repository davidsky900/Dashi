	...
	public static void main(String [] args) {
		...
		iterable.forEach(new Block<Document>() {
			...
			public void apply(final Document document) {
				previousItems.add(document.getString(ITEM_COLUMN));// read item
				previousRatings.add(document.getDouble(RATING_COLUMN));// read rating
			}
		});
                               /**
		var map = function() 
		...
		sb.append("function() {");
		// mapper function, check if item & ratings are match, if so, emit this user
		...
		// Get similar users' previous records order by similarity, similarity is defined as long as one user has
		// a same rating for a same item
